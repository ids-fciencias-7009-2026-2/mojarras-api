...
class CatApiClient(
    ...
    @Value("\${external.cat-api.base-url}") private val baseUrl: String,
    @Value("\${external.cat-api.api-key}") private val apiKey: String
) {

    private val logger = LoggerFactory.getLogger(CatApiClient::class.java)
    ...
    fun getBreeds(): List<ExternalBreedResponse> {
        return try {
            ...
            logger.error("Cat API failed", ex)
            emptyList()
        }
    }
}